...
open class Floaty: UIView {
	...
	*/
	...
	// Custom
	/** Padding from bottom that the floaty has when ht keyboard is open */
	@IBInspectable open var customOverKeyboardPaddingY: CGFloat = 0.0

	/** Padding from bottom that the floaty has when ht keyboard is closed */
	@IBInspectable open var customPaddingY: CGFloat = 0.0
	
	@IBInspectable open var autoHandlePosition: Bool = false
	
	/**
	If you created this object from storyboard or `initWithFrame`, this property set true.
	*/
	@IBInspectable open var isCustomFrame: Bool = false
	...
	*/
	...
	fileprivate var currentKeyboardHeight: CGFloat? = 0.0
	...
	*/
	...
	internal func deviceOrientationDidChange(_ notification: Notification) {
		...
		// print("Keyboard size on orientation change: ", currentKeyboardHeight)
		
		/// Update overlay frame for new orientation dimensions
		setOverlayFrame()
		
		if isCustomFrame == false {
			setRightBottomFrame(currentKeyboardHeight ?? 0)
		} else {
			size = min(frame.size.width, frame.size.height)
		}
		
		if autoHandlePosition {
			self.updateYProperly()
		}
	}
	
	internal func keyboardWillShow(_ notification: Notification) {
		currentKeyboardHeight = (notification.userInfo?[UIKeyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height
		
		
		guard (currentKeyboardHeight != nil) else { return }
		...
		// print("Updating on k show change Y to: ", UIScreen.main.bounds.height - self.size  - currentKeyboardHeight! - self.customOverKeyboardPaddingY)
	...
	internal func updateYProperly() {
		let paddingY = isCustomFrame ? customPaddingY : self.paddingY
		
		if currentKeyboardHeight == nil || currentKeyboardHeight! < CGFloat(1.0) {
			/*
			print("Updating for hidden keyboard")
			print("Screen H: ", UIScreen.main.bounds.height)
			print("Self size: ", self.size)
			print("Used Y padding", paddingY)
			*/
			self.updateY(UIScreen.main.bounds.height - self.size - paddingY)
		}
		else {
			/*
			print("Updating for shown keyboard")
			print("Screen H: ", UIScreen.main.bounds.height)
			print("Self size: ", self.size)
			print("Current keyboard height: ", currentKeyboardHeight)
			print("Custom padding over keyboard ", customOverKeyboardPaddingY)
			*/
			self.updateY(UIScreen.main.bounds.height - self.size  - currentKeyboardHeight! - self.customOverKeyboardPaddingY)
		}
		
	}
	
	internal func updateY(_ y : CGFloat) {
		self.frame = CGRect(
